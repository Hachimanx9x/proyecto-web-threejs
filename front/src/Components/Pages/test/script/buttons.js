...
export default class {
  ...
  get button() {
    let textpantalla = this.makeLabelCanvas(
      this.datalebel.size,
      this.datalebel.text
    ); //creo la textura a travez de un canvas
    let canvasTexture = new THREE.CanvasTexture(textpantalla); // vuelvo el canvas en una textura
    canvasTexture.minFilter = THREE.LinearFilter;
    canvasTexture.wrapS = THREE.ClampToEdgeWrapping;
    canvasTexture.wrapT = THREE.ClampToEdgeWrapping; // filtro la texutra
    let labelmaterials = new THREE.MeshBasicMaterial({
      map: canvasTexture,
      side: THREE.DoubleSide,
      transparent: true,
    }); // vuelvo la textura un material
    let labelgeo = new THREE.Mesh(
      new THREE.PlaneBufferGeometry(1, 1),
      labelmaterials
    ); // creo el plano con el materia donde estara el texto
    labelgeo.scale.x = this.scalelebel.x;
    labelgeo.scale.y = this.scalelebel.y; // le agrego sus dimenciones
