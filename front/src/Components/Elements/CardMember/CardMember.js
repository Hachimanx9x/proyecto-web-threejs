            ...
            <Fade show={show}>
                <div className="card o-card-member rounded">
                    <div className="text-center">
                        <img className="rounded-circle o-member-picture" alt="member profile" src={member.urlimage} />
                        <p>{member.name}</p>
                    </div>
                    ...
                    {remove === null || remove === undefined ?
                        <button className="z-depth-0 btn-block border-0 btn btn-primary font-weight-bold m-0" onClick={addMember}>Agregar</button> :
                        <><div className="d-flex justify-content-between">
                            <button className="bg-transparent border-0" onClick={deleteConfirmation}><FontAwesomeIcon icon={faTrash} className="text-danger" /></button>
                            <Link to={{ pathname: "/Dashboard/Contacts/" + member.name, contact: member }}><FontAwesomeIcon icon={faInfoCircle} style={{ border: "1px solid #007E33" }} className="bg-success text-white border-success rounded-circle" /></Link>

                        </div>
                            <div className="o-rol-select">
                                <select onChange={(e) => change(member, e.target.value)}>
                                    <option hidden>Rol del proyecto</option>
                                    <option value="Arquitecto de experiencia multimedia">Arquitecto de experiencia multimedia</option>
