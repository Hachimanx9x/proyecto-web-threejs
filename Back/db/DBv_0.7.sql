...
CREATE TABLE CONTENIDOS(id int primary key AUTO_INCREMENT, nombre varchar(50), descripcion varchar(200), url_documentacion varchar (50), bibliografica varchar(50), usariopropietario INT  , FOREIGN KEY(usariopropietario) REFERENCES USUARIOS(id)); 
...
CREATE TABLE PROYECTOS (id INT PRIMARY KEY AUTO_INCREMENT, nombre VARCHAR(350), descripcion VARCHAR(1350), estado VARCHAR(150), icon VARCHAR(1000), banner VARCHAR(1000), integrante INT , historia INT, practica INT, contenedor VARCHAR(1000),FOREIGN KEY(integrante) REFERENCES INTEGRANTES(id),FOREIGN KEY(historia) REFERENCES HISTORIAL(id),FOREIGN KEY(practica) REFERENCES PRACTICAS(id));
