...
let promesa = new Promise((res, rej) => {
  mariaDB.connect((error) => {
    if (error) {
      console.log(
        chalk.bold.red("error en la conexión de ") +
          chalk.bgRed.bold(" mariaDB ")
      );
      sqlite = new sqlite3.Database(dbPath, (err) => {
        ...
        vDB = false;
        console.log(
          chalk.white.bold("Estado de SQLITE es :") +
            chalk.white.bgBlue(" Conectado ")
        );
      ...
      res({ mariaDB, sqlite, vDB });
      console.log(
        chalk.white.bold("Estado de mariaDB es :") +
          chalk.white.bgBlue(" Conectado ")
      );
    }
  });
