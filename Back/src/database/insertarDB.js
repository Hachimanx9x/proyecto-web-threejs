...
funcionesDB.insertPerson=  async function(obj){
...
funcionesDB.insertLenguaje= async function(obj){
...
funcionesDB.insertContacts= async  function(obj){
...
funcionesDB.insertTools=  async function(obj){
    const {nombre, nivel} = obj; 
    await mariaDB.query(Query.insertTools(nombre, nivel), (err, rows, fields) => {
        if (!err) {    res.json({msj:"success" });
        } else {            res.json({msj:"error" });       }
    });
}
funcionesDB.insertAbility=  async function(tipo, descripcion, nivel,herramientaUsada){
...
funcionesDB.insertUser= async function(correoElectronico, urlHojaVida, contrasena, experiencia,contacto, persona,habilidad){
