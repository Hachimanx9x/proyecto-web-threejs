...
funcionesDB.eliminarproyecto = (proyecto) => {
  return new Promise((res, rej) => {
    buscarDB
      .obtenerdatosintgrantesdeproyecto(proyecto)
      .then((result) => {
        let integrantes = reorganizarintegrantes(result.API);

        buscarDB
          .obtenerentredeproyectoconconte(proyecto)
          .then((info) => {
            let entregalbes = filtrarinfopraeleminacioentregalbe(info.API);
            eliminaciondelistaintegrantes(integrantes, proyecto)
              .then((result2) => {
                console.log(
                  chalk.bgRed("|   |") + " lista integrantes eliminada"
                );
                eleminarintegrantes(integrantes)
                  .then((result3) => {
                    console.log(
                      chalk.bgRed("|   |") + " integrantes eliminada"
                    );
                    funcionesDB
                      .deleteProject({ id: proyecto })
                      .then((result4) => {
                        console.log(
                          chalk.bgRed("|   |") + " proyecto eliminada"
                        );
                        let archivos = archivosaeliminar(result.API);
                        let listaarchivos = archivos.concat(
                          entregalbes.contenidos
                        );

                        let set = new Set(listaarchivos.map(JSON.stringify));
                        let arrSinDuplicaciones = Array.from(set).map(
                          JSON.parse
                        );

                        res({
                          proyecto: proyecto,
                          archivos: arrSinDuplicaciones,
                        });
                      })
                      .catch((err4) => rej(err4));
                  })
...
funcionesDB.deleteListMember = (obj) => {
  return new Promise((res, rej) => {
    ...
    promesa.then((result) => {
      ...
      if (vDB) {
        mariaDB.query(
          Query.deleteListaIntegrantes(integrante, proyecto),
          (err) => {
            if (!err) {
              res({ msj: "success" });
            } else {
      ...
      if (vDB) {
        ...
        sqlite.all(Query.deleteallHerramientasMetodologia(), (err) => {
          if (!err) {
            ...
            rej({ msj: "deleteallToolsMetodology" });
          }
        });
      }
    });
      ...
      if (vDB) {
        ...
        sqlite.all(Query.deleteallListaentregables(), (err) => {
          if (!err) {
            ...
            rej({ msj: "deleteallListDeliverable" });
          }
        });
      }
    });
      ...
      if (vDB) {
        ...
        sqlite.all(Query.deleteallListaHerramientasMetodologia(), (err) => {
          if (!err) {
            ...
            rej({ msj: "deleteallListToolsMetodology" });
          }
        });
      }
    });
